...
Audit data lives in a dozen incompatible shapes. `auditnorm` maps logs from **ServiceNow**, **AWS CloudTrail**, **Okta**, **Splunk**, **GCP Cloud Audit**, **Azure Activity**, and **GitHub** into one common, OCSF/ECS-flavored schema so you can search, correlate, and report on activity across every system — and it can **auto-detect** which format each record is.
...
- **Adapters are plain functions** (`from_servicenow`, `from_cloudtrail`, `from_okta`, `from_splunk`, `from_gcp`, `from_azure`, `from_github`), so adding a new source is a few lines.
